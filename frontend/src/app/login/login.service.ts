...
export class LoginService {
  ...
  constructor(private http: HttpClient) {
    this.userUrl = '/api/code-shooting/v1/actions/getInfo';
  }
  public getUserInfo(user: object): Observable<any> {
    console.log(user);
    return this.http.post(`${this.userUrl}`, user);
  }

  public qrMd5() {
    return this.md5QrCode('TwoDIMAuth');
  }

  public verify(userId: any): Observable<any> {
    return this.http.post('/api/code-shooting/v1/actions/verify', {
      id: userId
    });
  }
  rotateLeft(lValue: number, iShiftBits: number) {
  ...
  numberRank(length) {
    ...
    for (var i = 0; i < length; i++) {
      rankstr += Math.floor(Math.random() * 10);
    }
    return rankstr;
  }
  ascii(char_s) {
  ...
  md5QrCode(type: string) {
    let qrCodeStr = type + ':';
    let date = new Date();
    let times = date.getTime();
    let key = this.md5(type + times + this.numberRank(10));
    qrCodeStr += key;
    qrCodeStr += ':';
    let value = this.md5(key + this.numberRank(10));
    qrCodeStr += value;
    return qrCodeStr;
  }
  seedCode(workno: number, imei: any) {
    let date = new Date();
    let times = date.getTime();
    let seedCodeStr = this.md5(times + workno + imei + this.numberRank(10));
    seedCodeStr += this.md5(seedCodeStr);
    seedCodeStr += this.md5(seedCodeStr);
    return seedCodeStr;
  }
  dynamicCode(seedCode: string, times: any) {
    let dynamicCodeStr = '';
    let a1 = this.md5(seedCode + times);
    let a2 = this.md5(seedCode + a1);
    let seed = a2.substr(0, 6).split('');
    let tmp;
    for (let i = 0; i < seed.length; i++) {
      ...
      if (!isNaN(tmp)) {
        ...
        dynamicCodeStr += this.ascii(tmp);
      }
    }
    return dynamicCodeStr;
  }
  verfiyCode(input: any) {
