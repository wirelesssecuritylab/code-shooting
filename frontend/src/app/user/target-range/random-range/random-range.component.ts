...
export class RandomRangeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.workspace = localStorage.getItem('target-range-ws') || DEFAULT_WORKSPACE;
    this.activatedRoute.queryParamMap.subscribe((paramMap) => {
      this.targetIdFromUrl = paramMap.get('targetId');
    });
    this.setColumns();
  ...
  initTagOptions() {
    ...
    this.manageService.getWorkSpaceLangDefect(this.workspace, language, "", true).subscribe(res => {
      ...
      this.mainCategory = '所有';
      this.initSubBugTypeOptions(this.mainCategory);
      this.plxMessageService.error('获取所属语言失败！', error.cause);
    })
  }
  ...
  */
  ...
  getTargetList() {
    let language = this.language != '所有' ? this.language : ''
    localStorage.setItem('target-range-ws', this.workspace);
    let reqBody: any = {
      name: 'query',
      parameters: {
    ...
    this.manageService.manageTargetApi(reqBody).subscribe(res => {
      ...
      if (res.result == 'success' && res.detail && Array.isArray(res.detail)) {
        ...
        if (!this.targetIdFromUrl) {
          ...
          this.targetData = targetList.filter(item => item.id == this.targetIdFromUrl);
          this.targetIdFromUrl = null;
        }


        // this.targetData = targetList.map(item => {
        //   if (!this.language && langs.indexOf(item['language']) < 0) {
        //     langs.push(item['language']);
        //     langOptions.push({label: item['language'], value: item.language});
        //   }
        //   return item;
