...
// RegisterPProfHandler adds several routes from package `net/http/pprof` to *echo.Echo object.
func RegisterPProfHandler(prefix string, e *echo.Echo) {
	routers := []struct {
		Method  string
		Path    string
	...
	for _, r := range routers {
		switch r.Method {
		case "GET":
			e.GET(strings.TrimPrefix(r.Path, prefix), r.Handler)
		case "POST":
			e.POST(strings.TrimPrefix(r.Path, prefix), r.Handler)
		}
	}
}
